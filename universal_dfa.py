# Universal DFA
# Template by Mark Bun and Huiwen He
# Implementation by Grace Yoon
...
class UniversalDFA(object):
    ...
    def simulate_DFA(self, encoded_DFA, input_string):
        ...
        list_of_states = [start_state]
        add_on = start_state
        i = 0
    
        while i < len(input_string):
            add_on = transitions.get((add_on, input_string[i]))
            list_of_states.append(add_on)
            i = i + 1

        # determine if our list of states ends in an accepting state
        state_set = set(list_of_states)
        final_states_set = set(final_states)
        decision = "reject"
        if (state_set.intersection(final_states_set)):
            decision = "accept"
        list_of_states.append(decision)
        
        # format transcript
        transcript = list_of_states
